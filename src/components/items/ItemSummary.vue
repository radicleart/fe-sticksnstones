    ...
    <div class="upload-preview mb-2">
      <div v-if="hasFile('music')">
        <div class="mb-4">
          ...
          <a class="text-success" href="#" @click.prevent="showHash = !showHash"> {{ itemSummary.item.attributes.artworkFile.name }}</a>
...
export default {
  ...
  methods: {
    ...
    hasFile: function (type) {
      if (type === 'music') {
        return (this.itemSummary.item.attributes.artworkFile) ? this.itemSummary.item.attributes.artworkFile.name : null
      }
