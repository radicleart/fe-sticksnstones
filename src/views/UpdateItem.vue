...
<div id="update-item" class="text-white" v-if="loaded && myAsset">
  <div class="container" :key="componentKey">
    <div class="row mt-4">
      <div class="col-12">
        <h1>Defining your NFT File</h1>
        <p>Your NFT file is <a :href="myAsset.nftMedia.artworkFile.fileUrl" target="_blank">lives here</a></p>
        <p>Filename is {{myAsset.nftMedia.artworkFile.name}} and {{myAsset.nftMedia.artworkFile.size}} Kb</p>
        <p>Please upload a cover image (png/jpg) or short video (mp4, less than 5M) to display your file on the Risidio Xchange.</p>
      </div>
    </div>
    <div v-if="isNotImage()">
      <media-handler :videoOptions="videoOptions()" :uploadState="uploadState" :nftMedia="item.nftMedia" :mediaTypesAllowed="mediaTypesAllowed" @updateMedia="updateMedia" @deleteMediaItem="deleteMediaItem"/>
    </div>
    <div class="row mt-4">
  ...
  methods: {
    videoOptions () {
      ...
      const videoOptions = {
        ...
        hideArtworkClip: true,
        hideArtworkFile: true,
        autoplay: false,
        showMeta: true,
        controls: true,
    ...
    isNotImage: function () {
      ...
      if (item && item.nftMedia.artworkFile) {
        return item.nftMedia.artworkFile.type.indexOf('image') === -1
      }
    },
